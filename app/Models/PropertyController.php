...
$rand_key = "";
	...
	public function search_property()
	{
		...
		if(!empty($_REQUEST['property_type']) || !empty($_REQUEST['bedrooms']) || !empty($_REQUEST['bathrooms']) || !empty($_REQUEST['min_price']) || !empty($_REQUEST['max_price']) || !empty($_REQUEST['search_region']) || !empty($_REQUEST['builder']) || !empty($_REQUEST['page']) || !empty($_REQUEST['main_regionchange']))
		{
			...
			$query = Property::with( ['builder_detail' => function ($q) {
   $q->where('trash' , 'no');	
	...
	public function get_property_html($prop_data,$page,$lastpage,$numrows,$currentpage,$rows_per_page)
	{
	 ...
	 /* 	echo '<pre>';
       ...
       $html.=   '<div class="grey_bg"><!--white box!-->
        	<div class="model_left">
            	<div class="star">';
				  $check_save_prop =  App\Models\SaveProperty::check_new_save_prop($prop_val['id']) ; 
                            $html.=	'<a href="javascript:void(0);" rel="'.$prop_val['id'].'" class="save_property_new" >';
							  if($check_save_prop != '0') { $html.= '<img src="'.url().'/assets/img/star_hover.png" data-save_'.$prop_val['id'].'="Saved" id="save_src_'.$prop_val['id'].'">'; 
							  } else { 
							  $html.= '<img src="'.url().'/assets/img/star.png" data-save_'.$prop_val['id'].'="Save" id="save_src_'.$prop_val['id'].'">';
							  }
							  $html.= '</a>
				</div>
            	<div class="model_img">';
				...
				if(!empty($prop_val['property_gallery'])) {
					foreach($prop_val['property_gallery'] as $prop_img) {
						...
						$html.= '<a href="'.url().'/propertydetail/'.$prop_val['id'].'"><img src="'.url().'/uploads/property_gallery/'.$prop_img['image'].'" /></a>';
					}
            ...
            <div class="model_right"><!--right section!-->
            	<a href="'.url().'/builder-detail/'.$prop_val['builder_detail']['builder_id'].'"><div class="c_logo">';
				
				 if(!empty($prop_val['builder_detail']['logo'])) { 
				 $html.= '<img src="'.url().'/uploads/builder_logo/'.$prop_val['builder_detail']['logo'].'">' ; 
				 } 
				 
				 $html.= '</div></a>
            	<a href="'.url().'/propertydetail/'.$prop_val['id'].'"><h4>'.$prop_val['property_title'].'</h4></a>
                <h5>From $ '.number_format($prop_val['price'],2).' <i class="fa fa-info-circle" data-toggle="tooltip" data-placement="top" title="The price or price range shown here is indicative only and will vary depending on the final inclusions, location of the build, the house façade and other customisations selected."></i></h5>
                <ul class="icon_set i_set2">
                 	<li>
                            	<i><img src="'.url().'/assets/img/bed.png"> <span><em>'.$prop_val['bedrooms'].'</em> beds</span></i>
                             </li>
                           <li>
                            	<i><img src="'.url().'/assets/img/bath.png"> <span><em>'.$prop_val['bathrooms'].'</em> bath</span></i>
                             </li>
                            <li>
                            	<i><img src="'.url().'/assets/img/sofa.png"> <span><em>'.$prop_val['living'].'</em> living</span></i>
                             </li>
                           <li>
                            	<i><img src="'.url().'/assets/img/size.png"> <span><em>'.$prop_val['housesize'].'</em> sq </span></i>
                             </li>       	
                 </ul>
                 <p>';
...
$html.= '</p>
                 <ul class="button_set">
                        	<li>
                            	<a href="javascript:void(0);" class="open_quicklook"   data-target=".quick-look-modal" data-id = "'.$prop_val['id'].'"><img src="'.url().'/assets/img/quick.png" class="quick-img">Quick look</a>
                            </li>
                            <li>
                            	<a href="javascript:void(0);" class="open_enquirybox" value="Enquire to Builders" data-toggle="modal" data-target=".bs-example-modal-lg" data-id = "'.$prop_val['id'].'" ><img src="'.url().'/assets/img/message.png">Message</a>
                            </li>';
							 $check_save_prop =  SaveProperty::check_save_prop($prop_val['id']) ;
						$html.='	
                            <li>
                            	<a href="javascript:void(0);" rel="'.$prop_val['id'].'" class="save_property" >';
								  if($check_save_prop != '0') { $html.= '<img src="'.url().'/assets/img/comapre_hover.png" id="compare_src_'. $prop_val['id'].'">'; } else { $html.= '<img src="'.url().'/assets/img/comapre.png" id="compare_src_'.$prop_val['id'].'">'; } 
								  
								$html.= '<span id="save_text_'.$prop_val['id'].'">';
								  if($check_save_prop != '0') { $html.= 'Compared' ; } else {$html.= 'Compare' ; } 
...
/* 	  echo '<pre>';
	...
	public function propertydetail($prop_id){
		...
		$house_land_query = Property::where(array('property_type'=>'2','user_id'=>$user_id))->get(array('id','house_land_address'));
 ...
 <input type="radio" name="secured_option3" value="No" >No
            ...
            <p class="cp-left agree">
              ...
              <span>I have read and agree to the iCompareLoans Website<a href="http://www.icompareloans.com.au/" target="_blank"> Terms &amp; Conditions</a>,
	 ...
	 /* 	echo '<pre>';
       ...
       $html.=   '<div class="grey_bg"><!--white box!-->
            ...
            <div class="model_right"><!--right section!-->
            	...
            	<a href="'.url().'/propertydetail/'.$prop_val['id'].'"><h4>'.$prop_val['property_title'].'</h4></a>
                <h5>From $ '.number_format($prop_val['price'],2).' <i class="fa fa-info-circle" data-toggle="tooltip" data-placement="top" title="The price or price range shown here is indicative only and will vary depending on the final inclusions, location of the build, the house façade and other customisations selected."></i></h5>
				<span class="length">House: '.$prop_val['housesize'].'sqm | Land: '.$prop_val['land_size'].'sqm</span>
                <ul class="icon_set i_set2">
                 	<li>
                            	<i><img src="'.url().'/assets/img/bed.png"> <span><em>'.$prop_val['bedrooms'].'</em> beds</span></i>
                             </li>
    ...
    <div class="ql-right">
        ...
        <div class="ql-details">
            ...
            <div class="qld-right">
             ...
             <a class="view-full" href="'.url().'/propertydetail/'.$prop_arr[0]['id'].'" target="_blank">View full detail <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
            </div>
            <div class="clr"></div>
        </div>
