                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                           <?php echo $title; ?>
						   <input type="button"  value="Sort Featured House and land" onclick="window.location.href='<?php echo url(); ?>/admin/house-and-land/sort_featured'"class="btn btn-primary padd_button">
							<input type="button"  value="Add House and Land" onclick="window.location.href='<?php echo url(); ?>/admin/house-and-land/create'"class="btn btn-primary padd_button">
                        </div>
                        <!-- /.panel-heading -->
                        <div class="panel-body">
						<div class="row" style="margin:10px;">
						<p style="font-size: 15px;font-weight: bold;">Total House & Land <?php echo $numrows; ?></p>
				...
				?>
										...
										<tr>
                                            ...
                                            <td>
												...
												<?php //} ?>
												<button class="btn btn-primary btn-mini" <?php if($properties_val['status'] == "1"){  ?> onclick="window.location.href='<?php echo url(); ?>/admin/house-and-land/<?php echo $properties_val['id']; ?>/activate/0'" <?php } else { ?> onclick="window.location.href='<?php echo url(); ?>/admin/house-and-land/<?php echo $properties_val['id']; ?>/activate/1'" <?php } ?> > <?php if($properties_val['status'] == "1"){  ?>On<?php }else{ ?>Off<?php } ?></button>
											</td>
                                            <td>
												<button class="btn btn-info" type="button" onclick="window.location.href='<?php echo url(); ?>/admin/house-and-land/view/<?php echo $properties_val['id']; ?>'">View Detail</button>
												<button class="btn btn-primary btn-mini" onclick="window.location.href='<?php echo url(); ?>/admin/house-and-land/edit/<?php echo $properties_val['id']; ?>'">Edit</button> 
												<button class="btn btn-danger btn-mini" onclick="javascript:delete_house_land(<?php echo $properties_val['id'] ;  ?>)" >Delete</button>
												<button class="btn btn-primary btn-mini" onclick="window.location.href='<?php echo url(); ?>/admin/house-and-land/featured/<?php echo $properties_val['id']; ?>'" ><?php if($properties_val['featured'] == "Yes"){  ?><i class="fa fa-star" style='color:#FFD700;'></i> <?php }else{ ?><i class="fa fa-star" ></i><?php } ?></button>
											</td>                                            
                                        </tr>
...
echo   $pages ; //PAGINATION LINKS
	...
	function delete_house_land(prop_id)
	{
		if(prop_id)
		{
			var status = confirm('Are you sure you want to delete House and Land.');
			if(status)
			{
				window.location.href='<?php echo url(); ?>/admin/house-and-land/delete/'+prop_id;
			}
		}
	}
