            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
						...
						list($controller, $method) = explode('@', $currentAction);
						 ...
						 if($controller == 'ContentController' && $method == 'edit_blog') {
						...
						?>
                                    <form method="post" action="<?php echo url(); ?>/admin/blog/update/<?php echo $content_arr['id']; ?>" enctype="multipart/form-data">
									 <input type="hidden" name="_token" value="<?php echo csrf_token() ; ?>">
										...
										<div class="form-group">
                                           ...
                                           <select class="builder_location" multiple  name="category_id[]">
										   <option value="0">None</option>
										  		<?php  
											foreach($category_arr as $cat_val) {
											if(in_array($cat_val['id'],$cat_arr)) { ?>
												 <option value="<?php echo $cat_val['id']; ?>" selected><?php echo $cat_val['category_title']; ?></option>
										<?php 	} else {
										   ?>
										   <option value="<?php echo $cat_val['id']; ?>"><?php echo $cat_val['category_title']; ?></option>
										   <?php  } } ?>
										  </select>
                                        </div>
                ...
                CKEDITOR.replace('desc');
										...
										<?php if(!empty($content_arr['featured_image'])) {   ?>
                                            <label>Image</label>
                                            <img src="<?php echo url(); ?>/uploads/featured_images/<?php echo $content_arr['featured_image'];  ?>"  width="50" height="50"/>
										<?php } ?>
										</div>
						...
						<?php } else if($controller == 'ContentController' && $method == 'create_blog') { ?>
						  <div class="panel-body">
						<?php 
						/*  echo '<pre>';
						print_r($landestate_arr); 
						...
						?>
                                    <form method="post" action="<?php echo url(); ?>/admin/blog/add" enctype="multipart/form-data">
									 <input type="hidden" name="_token" value="<?php echo csrf_token() ; ?>">
									 	<div class="form-group">
                                            <label>Title</label>
