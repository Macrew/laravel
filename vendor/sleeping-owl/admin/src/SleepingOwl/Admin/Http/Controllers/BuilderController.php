...
class BuilderController extends Controller
{
	...
	//dd($input);
		...
		{
			$check = $this->check_username($_POST['email'],$_POST['username']);
			if ($check['status']=='true')
			{
				$validator->errors()->add('username', $check['msg']);
			}
		});
		...
		/*  echo '<pre>';
		 ...
		 if(!empty(Input::file('inclusion_brochure'))) {
		  ...
		  $builders->inclusion_brochure = $fileName;
		  // sending back with message
		 // Session::flash('success', 'Upload successfully'); 
		  //return Redirect::to('upload');
		}
	...
	public function check_username($email,$username)
	{
		$data = array();
		if($username != "") {
		$user = User::where(['username' => $username,'email'=>$email]);
		if($user->count() == 1)
		{
			$data['status'] = 'false';
			$data['msg'] = '';
	...
	//dd($input);
		...
		$builder = new BuilderDetail;
		 ...
		 if(!empty(Input::file('inclusion_brochure'))) {
		  ...
		  $fileName = rand(11111,99999).'-incs'.'.'.$extension; // renameing image
		  Input::file('inclusion_brochure')->move($destinationPath, $fileName); // uploading file to given path
		  $builder->inclusion_brochure = $fileName;
		  // sending back with message
		 // Session::flash('success', 'Upload successfully'); 
		  //return Redirect::to('upload');
		}
	...
	{
		$user = BuilderDetail::where(['builder_id' => $builder_id])->first();
		if($user->trash == 'yes') {
			$user->trash = 'no';
		} else {
			$user->trash = 'yes';
