...
class PropertyController extends Controller
{
	...
	public function get_properties()
	{
		...
		if(!empty($_REQUEST['main_regionchange']) && $_REQUEST['main_regionchange'] != 'select-region')
			{
			...
			if(!empty($_REQUEST['keyword']))
			{
				...
				$query->orWhereHas('builder_detail', function($q) use($data){
		...
		$page	=	"" ;
			$lastpage	=	"" ;
			$numrows  = "" ;
			$total_prop = $this->count_search_property($main_regionchange,$keyword);
	...
	public function count_search_property($main_regionchange="",$keyword="")
	{
		$where_arr = array('user_type'=>'Builder');
		
		$query = Property::with('builder_detail');
		//$results = User::where($where_arr)->builders;
		if(!empty($main_regionchange) || !empty($main_regionchange)) {
	...
	public function get_house_lands()
	{
		...
		$data['title'] = "House and Land";
		return view(AdminTemplate::view('pages.house-and-land'), $data);
	}
	
	
	public function count_house_land($main_regionchange="",$keyword="")
	{
	...
	public function edit_house_land($prop_id)
	{
		$results = Property::where(['id' => $prop_id])->first();
		//$results = User::with('builders')->where($where_arr)->get();
		$data['property_arr'] = $results->toArray();
		...
		$data['title'] = "Edit House & Land";
		
		return view(AdminTemplate::view('pages.edit_house_lands'), $data);
	}
	...
	public function update_property($prop_id, Request $request)
	{
		...
		{
			$check = $this->vldte_dsply_hmes_adres($_POST['suburb_address']);
			if ($check['status']=='false')
			{
				$validator->errors()->add('suburb_address', $check['msg']);
			}
		});
		...
		/*  echo '<pre>';
		 ...
		 if(!empty(Input::file('inclusion_brochure'))) {
		  if (Input::file('inclusion_brochure')->isValid()) {
		  $destinationPath = 'uploads/brochure'; // upload path
		  $extension = Input::file('inclusion_brochure')->getClientOriginalExtension(); // getting image extension
		  $fileName = rand(11111,99999).'-incs'.'.'.$extension; // renameing image
		  Input::file('inclusion_brochure')->move($destinationPath, $fileName); // uploading file to given path
		  $properties->inclusion_brochure = $fileName;
		  // sending back with message
		 // Session::flash('success', 'Upload successfully'); 
		  //return Redirect::to('upload');
		}
		 ...
		 $properties->fixed_site_works = $_POST['fixed_site_works'];
		 ...
		 $properties->property_type = $_POST['property_type'];
		 $lat = Session::get('lat');
		 $lng = Session::get('lng');
		 $properties->suburb_address = $_POST['suburb_address'];
		 $properties->suburb_geo_lng = $lng;
		 $properties->suburb_geo_lat = $lat;
		/*  if($_POST['property_type'] == '2')
		 {
			$properties->property_type = $_POST['property_type'];
			 $properties->land_size = $_POST['land_size'];
			$properties->house_land_address = 	$_POST['house_land_address'];
		 } else {
			$properties->property_type = $_POST['property_type'];
			 $properties->land_size = "";
			$properties->house_land_address = 	"";
		 } */
	...
	public function update_house_land($prop_id, Request $request)
	{
		$input = Input::all();
		//dd($input);
		$rules = array(
		...
		// if the validator fails, redirect back to the form
		if ($validator->fails()) {
				return Redirect::to('admin/house-and-land/edit/'.$prop_id)
				->withErrors($validator); // send back all errors to the login form
		} else {
		 $properties = Property::where(['id' => $prop_id])->first();
		/*  echo '<pre>';
		...
		Session::flash('update_message', 'House and land Detail has been updated');
	...
	public function create_house_land()
	{
		$data['title'] = "Create House and Land";
		$properties = Property::all();
		$data['property_arr'] = $properties->toArray();
 ...
 */
	...
	public function add_house_land(Request $request)
	{
	...
	//dd($input);
		...
		// if the validator fails, redirect back to the form
		if ($validator->fails()) {
				return Redirect::to('admin/house-and-land/create')
				->withErrors($validator) // send back all errors to the login form
				->withInput();
		} else {
		 $properties = new Property;
		...
		/*  echo '<pre>';
         ...
         $properties->save();
		 $lastInsertedId= $properties->id;
		Session::flash('save_message', 'House and land detail has been added');

		return Redirect::to('admin/house-and-land/edit/'.$lastInsertedId);
	...
	public function delete_house_land($prop_id)
	{
		...
		if($propdel->delete())
		{
			...
			Session::flash('delete_message', 'House and land & its content has been deleted');
			return redirect()->back();
		}
	}
	...
	public function get_galleries()
	{
		...
		$results = Property::where('property_type','1')->orWhere('property_type','3')->get();
		//$results = User::where($where_arr)->builders;
 ...
 */
	...
	public function get_house_land_galleries()
	{
		
		$results = Property::where('property_type','2')->get();
		//$results = User::where($where_arr)->builders;
 ...
 */
		//$data['properties_arr'] = $properties->toArray();
		$data['title'] = "House and Land Photo Gallery";
		return view(AdminTemplate::view('pages.house_land_photo_gallery'), $data);
	}
	...
	public function create_house_land_gallery()
	{
		$data['title'] = "Create Gallery";
		...
		return view(AdminTemplate::view('pages.edit_house_land_gallery'), $data);
	}
	
	public function ajax_get_builder_properties()
	{
		$builder_id = $_POST['builder'];
		$query = Property::where(['user_id' => $builder_id]);
	...
	public function ajax_get_builder_house_land()
	{
		$builder_id = $_POST['builder'];
		$query = Property::where(['user_id' => $builder_id]);
		$query->where('property_type','2');
	...
	public function edit_house_land_gallery($prop_id)
	{
		$results = Property::with('property_gallery')->where(['id' => $prop_id])->get();
		//$results = User::with('builders')->where($where_arr)->get();
		$data['prop_arr'] = $results->toArray();
	...
	public function add_more_house_land_images($propid)
	{
		$data['propid'] = $propid;
		$data['title'] = 'Edit Gallery';
		return view(AdminTemplate::view('pages.edit_house_land_gallery'), $data);
	...
	{
		$results = Property::with('builder_detail','property_gallery','property_floor_plans','property_inclusions')->where(['id' => $prop_id,'property_type'=>'2'])->get();
		//$results = User::with('builders')->where($where_arr)->get();
		$data['prop_arr'] = $results->toArray();
		$inc = Inclusion::where(['parent_id'=>'0'])->get();
		$data['inc_arr']  = $inc->toArray();
		$data['title'] = 'View House and Land';
		return view(AdminTemplate::view('pages.view_house_land'), $data);
	}
	...
	public function featured_house_land($prop_id){
		$properties = Property::where(['id' => $prop_id])->first();
		//$results = User::with('builders')->where($where_arr)->get();
		$properties_arr = $properties->toArray();
		$featured = '';
		...
		$properties->featured = $featured;
		if($properties->save()){
			Session::flash('delete_message', 'House and land has been changed to featured.');
			return redirect()->back();
		}else{
			Session::flash('delete_message', 'Currently not able change House and land to featured. Please try again.');
			return redirect()->back();
		}
	}
	...
	public function sort_properties()
	{
		...
		$query = Property::with('builder_detail');
		$query->where(array('featured'=>'Yes'));
		$query->where('property_type','1');
		$query->Orwhere('property_type','3');
		$results = $query->orderBy('featured_order','Asc')->get();
		//$results = User::where($where_arr)->builders;
 ...
 */
	...
	public function sort_house_land()
	{
		$where_arr = array('user_type'=>'Builder');
		
		$results = Property::with('builder_detail')->where(array('featured'=>'Yes','property_type'=>'2'))->orderBy('featured_order','Asc')->get();
		//$results = User::where($where_arr)->builders;
 ...
 */
		//$data['properties_arr'] = $properties->toArray();
		$data['title'] = "Sort House and Land";
		return view(AdminTemplate::view('pages.sort_house_land'), $data);
	}
	...
	public function activate_house_lands($id){
		$uri = Request::path();
		$url = explode('/',$uri);
		DB::table('property')
            ->where('id', $id)
            ->update(['status' => $url[4]]);
        return Redirect::to('admin/house-and-lands');
	}
	...
	public function sort_house_land_gallery($prop_id)
	{
		$results = PropertyGallery::where(['property_id' => $prop_id])->orderBy('gallery_order', 'Asc')->get();
		//$results = User::with('builders')->where($where_arr)->get();
		//echo '<pre>';print_r($results->toArray());echo '</pre>';die;
		...
		return view(AdminTemplate::view('pages.edit_house_land_gallery_order'), $data);
	}
