 $.ajaxSetup({
   headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') }
    ...
    $(document).ready(function () {	
		...
		$('.toggle-inc').on('click',function(){
		$('.filter-inc').slideToggle();
		});
		  var base_url = $('#base_url').val();
		$('.builder-carasouel').slick({
  infinite: false,
  speed: 300,
  autoplay: true,
  slidesToShow: 5,
  slidesToScroll: 1,
  prevArrow: '<div class="arrow_left_caraso"><img src="'+base_url+'/assets/images/featured-strip-left-arrow.png" /></div>',
nextArrow: ' <div class="arrow_right_caraso"><img src="'+base_url+'/assets/images/featured-strip-right-arrow.png" /></div>'
});
		$('#right-menu').sidr({
			  name: 'sidr-right',
			  side: 'right'
...
var base_url = $('#base_url').val();
$('.model_img').slick({               
prevArrow: '<div class="arrow_left1"><img src="'+base_url+'/assets/images/featured-strip-left-arrow.png" /></div>',
nextArrow: ' <div class="arrow_right1"><img src="'+base_url+'/assets/images/featured-strip-right-arrow.png" /></div>'
});
...
var url = base_url+'/property/save_property';
	...
	var save_text = $('#comp_text_'+prop_id).val();
var datastring = 'prop_id='+prop_id+'&save_text='+save_text;
		$.ajax({
		type:'post',
		url:url,
...
{
	$('#house-div').on('click',function(event){
	event.stopPropagation();
	...
	$('#house-land-div').on('click',function(event){
	event.stopPropagation();
    ...
    $('#enquire_forms').attr("action",url);
      ...
      nextArrow:" <a href='javascript:void(0)' class='ql-arrow ql-arrow-right'><i class='fa fa-angle-right' aria-hidden='true'></i></a>",
				...
				$('#submit_call_back').on('click',function(){
					...
					var contact_time = $('#pop_contact_time').val();
					...
					else if($('#pop_terms-conditions').is(':checked') == false)
					{
						alert('Please accept terms & conditions');
						status = false;
					}
		if(status == true) {
		var  property_id = $('#pop_prop_id').val();
...
function submit_feedback_form() { 
 ...
 if($('#biggest_impact').val() == 'select impact')
  ...
  jQuery('.loading').addClass('loaderimg');
    ...
    jQuery('.loading').removeClass('loaderimg');
			if(data == 'success'){
				$('#message_success').text('Your feedback has been sent successfully');
				$('#message_success').show();
				$('#message_error').hide();
	...
	//console.log(url);
 	...
 	$.ajax({
		...
		if(data.regfail) {
			...
			$('#message_error1').css('display','block')
			$('.reg_error').text('');
			$('#successMessage').empty();          
        } 
